...
One can view the annotation of a model component as a statement in the form of a 'triple': <subject, predicate, object>. The list of qualifiers below describe the "predicate" or relationship between the subject and the object: The modeling entity that is part of the model (the subject), and the entity described by the annotation resource (the object). Below, we provide some example annotations that use these qualifiers with model entities and annotation resources. See also the OMEX metadata specification for additional examples. 

The namespace for these qualifiers is 'http://biomodels.net/model-qualifiers/'.
